...
export function VideoChatRoom({ roomId: roomIdProp, interest, nickname, myCountry, socket, isQueuing, onLeave, onFindNewPartner, onJoined }) {
  ...
  const gridClass = count <= 1 ? 'video-grid-1' : 'video-grid-2';
  ...
  return (
    <div className="flex flex-col h-screen bg-realm-void">
      ...
      <div className="flex-1 flex min-h-0">
        <div className={`flex-1 flex flex-col p-2 ${showChat ? 'max-w-[calc(100%-280px)]' : ''}`}>
          <div className={`flex-1 video-grid ${gridClass} min-h-0`}>
            <div className="video-tile">
              <video
                ...
                muted
                playsInline
                className={`mirror ${cameraOff ? 'opacity-30' : ''}`}
              />
              {cameraOff && !isQueuing && (
                <div className="absolute inset-0 flex items-center justify-center bg-realm-surface/80">
                  <span className="text-realm-muted text-sm">Camera off</span>
                </div>
              )}
              <div className="label">{countryToFlag(myCountry) ? `${countryToFlag(myCountry)} ` : ''}You · {nickname}</div>
            </div>
            {isQueuing ? (
              <div className="video-tile flex flex-col items-center justify-center bg-realm-card/50 border border-realm-border">
        ...
        {showChat && !isQueuing && (
          <div className="w-[280px] flex flex-col border-l border-realm-border bg-realm-surface">
            <div className="p-2 border-b border-realm-border"><span className="text-sm font-medium text-realm-muted">Chat</span></div>
...
function RemoteVideo({ stream, nickname, country, socketId }) {
  ...
  return (
    <div className="video-tile">
      ...
      <div className="label">{flag ? `${flag} ` : ''}{nickname || 'Stranger'}</div>
    </div>
  );
}
